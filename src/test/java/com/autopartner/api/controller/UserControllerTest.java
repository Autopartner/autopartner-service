...
public class UserControllerTest extends AbstractControllerTest {
  ...
  void get_ValidUserId_ReturnsUser() throws Exception {
    User user = UserFixture.createUser();
    long userId = 1L;
    when(userService.findById(eq(userId), any())).thenReturn(Optional.of(user));
  ...
  void get_InvalidUserId_ReturnsError() throws Exception {
    long userId = 1L;
    when(userService.findById(eq(userId), any())).thenReturn(Optional.empty());
  ...
  void update_InvalidRequest_ReturnsError() throws Exception {
    ...
    UserRequest request = createUserRequest();
    long id = 1L;
    when(userService.findById(eq(id), any())).thenReturn(Optional.of(user));
