...
public class TaskServiceTest {
  ...
  void findByCategoryIdAndName() {
    ...
    when(repository.findIdByCategoryIdAndName(eq(name), eq(categoryId), eq(companyId)))
        .thenReturn(Optional.of(task.getId()));

    service.findIdByCategoryIdAndName(name, categoryId, companyId);

    verify(repository).findIdByCategoryIdAndName(nameCaptor.capture(), taskIdCaptor.capture(), companyIdCaptor.capture());
